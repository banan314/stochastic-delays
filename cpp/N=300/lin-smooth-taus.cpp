...
double fA_preprocess(const signed short int &x, const double &omega)
{
  return double(N / 4) * (tanh(-omega * (x - d)) + 1);
}

double fB_preprocess(const signed short int &x, const double &omega)
{
  return double(N / 4) * (tanh(omega * (x + d)) + 1);
}
...
void preprocess()
{
  ...
  {
    fA_Preprocessed[x + N / 2] = fA_preprocess(x, 1.0);
    fB_Preprocessed[x + N / 2] = fB_preprocess(x, 1.0);
  }
}
...
int main()
{
  ...
  fileStream << "./csv/N=" << N << "/lin-smooth-taus-d" << d << ".csv";
