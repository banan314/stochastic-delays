...
int main()
{
    ...
    ss << "./csv/omegas/N=20/lin-step-tau-" << tauA << "-d-" << d << ".csv";
    ...
    for (int i = 0; i <= 50; ++i)
    {
        omega = 0.0f + (float)i / 50.0f;
        length = run(i);
        avg = mean(length);
        io << (int)d << "," << tauA << "," << omega << "," << avg << endl;

        ss.str("");
        // -" << tauA << " - d - " << d << ".csv "
        ss << "./csv/omegas/N=20/histogram/lin-step-histogram" << "-tau=" << tauA << "-d=" << d << "-omega" << omega <<  ".csv";
