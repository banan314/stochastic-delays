...
short int increase(const long long &current)
{
...
short int decrease(const long long &current)
{
    short int y = x[current];
    y -= 1;
    if (y < -N / 2)
    {
        y = -N / 2;
...
double fA_preprocess(const signed short int &x, const int &d, const double &omega)
{
    return double(N / 4) * (tanh(-omega * (x - d)) + 1);
}

double fB_preprocess(const signed short int &x, const int &d, const double &omega)
{
    return double(N / 4) * (tanh(omega * (x + d)) + 1);
}
...
int main() {
  ...
  auto prefix = "lin-smooth";
  fileStream << "./csv/N=300/" << prefix << "-ds-tau" << tauA << ".csv";
