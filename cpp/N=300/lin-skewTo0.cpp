...
short int increase(long long current)
{
...
short int decrease(long long current)
{
    short int y = x[current];
    y -= 1;
    if (y < -N / 2)
...
double fB_preprocess(signed short int x, const int &d)
{
    return x >= -d ? N / 2 : double(N / 2 * x + (N / 2) * (N / 2)) / double(N / 2 - d);
}
...
double fB(signed short int x, const int &dNumber)
{
